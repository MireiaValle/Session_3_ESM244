...
ggplot(eco_clip) +
  geom_sf(data = ca_counties, 
          fill = "gray90", 
          color = "gray80", 
          size = 0.2) + # First add gray California
  geom_sf(aes(fill = region), 
          color = "NA") + # ...then add eco-regions (clipped)
  scale_fill_manual(values = c("darkolivegreen2","darkolivegreen","gold2")) + # Change color scheme
  coord_sf(xlim = c(-121,-119), 
           ylim = c(33.5,35.5)) + # Crop plotting area
  geom_point(aes(x = -119.6982, 
                 y = 34.4208), size = 2) + # Add a point for SB City
  geom_text(x = -119.6982, 
            y = 34.35, label = "Santa Barbara") + # Then add a label for it
  theme_minimal() + # Update theme
  theme(legend.position = c(0.5,0.15)) +# Move the legend
  labs(x = "",
       y = "", 
       title = "Santa Barbara County Eco-Regions")
```
...
animal_map <- tm_basemap("Esri.OceanBasemap") +
  tm_shape(animals_sf) +
...
```{r}
ca_tot_map <- tm_basemap("Esri.OceanBasemap") +
  tm_shape(ca_tot) +
  tm_fill("n") 
